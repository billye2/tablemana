  ...
  return (
    <div className="space-y-3 rounded-xl border border-zinc-200 bg-white p-4 sm:p-5">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          onClick={copy}
          className="inline-flex min-h-11 items-center rounded-lg bg-zinc-900 px-4 text-sm font-medium text-white sm:min-h-10"
        >
          {copied ? "Copied" : "Copy link"}
        </button>
        <a
          href={`/counter/${slug}`}
          className="inline-flex min-h-11 items-center rounded-lg border border-zinc-300 px-4 text-sm font-medium text-zinc-700 sm:min-h-10"
        >
          Open counter here
        </a>
        <button
          disabled={pending}
          onClick={rotate}
          className="ml-auto inline-flex min-h-11 items-center rounded-lg border border-red-300 px-4 text-sm font-medium text-red-600 disabled:opacity-50 sm:min-h-10"
        >
