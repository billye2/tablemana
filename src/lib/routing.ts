...
import { ownerCookieName, ownerSlugFromPath } from "./owner-token";

/**
 * Request routing that runs inside the proxy, kept Clerk-free so it stays
 * unit-testable:
 *
 * - Counter and legacy dashboard links carry ?key= once; park it in an
 *   HttpOnly cookie and drop it from the URL so the key stops leaking via
 *   history/referrers/logs.
 * - Hostname-based tenancy (PLAN.md §7): `{slug}.<root>` serves the tenant
 *   site by rewriting to /t/{slug}/...; the root domain serves marketing,
 *   onboarding, dashboard, and counter.
 */
